...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  type ImageGenerationConfig = {
    ...
    generator: ({ location, variant, dimensions, fontColor }: { location: string; variant: EnumColorVariant; dimensions: ImageDimensions; fontColor: EnumColorHex }) => string;
  }
...
for (const imageGen of IMAGE_GENERATORS) {
    ...
    const html = imageGen.generator({ location, variant: imageGen.variant, dimensions: imageGen.dimensions, fontColor: imageGen.fontColor });
    ...
    zip.folder(`GDG ${location} - Brand Assets/${imageGen.folder}`)?.file(imageGen.name, buffer);
...
res.setHeader('Content-Disposition', 'attachment; filename=GDG_Brand_Assets.zip');
